    ...
    Request() {
      ...
      const interval = setInterval(() => {
        if (isSendedandrecived) {
          ...
          window.frontendData.language.chapters[1].listThemes[4].task.tryCount++
          window.frontendData.language.chapters[1].listThemes[4].task.timeOnSolutionInSeconds += this.timeSpended
          ...
          this.$toasted.error("Некоректные данные! Время на решение: " + this.timeSpended +
              " c. Номер попытки: " + window.frontendData.language.chapters[1].listThemes[4].task.tryCount + ".",{
...
\t}
  ...
  beforeDestroy() {
    ...
    let themeToClose = document.getElementById('java-2-chapter-3-them-3-subtheme');
