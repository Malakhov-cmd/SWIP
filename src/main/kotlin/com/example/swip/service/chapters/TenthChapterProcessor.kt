...
class TenthChapterProcessor(
    ...
    fun tenthChapterAnswers(
        ...
        val fourthChapter = javaLanguage.chapters[9]
    ...
    private fun checkerUserAnswerIfDefaultBehavior(
        ...
        val chapter = language.chapters[9]
    ...
    private fun checkToAddAchive(
        ...
        if (chapter.chapterProgress > 99) {
            if(user.achivesList.stream().filter{ it.name == "endedChapter10"  }.count() == 0L){
                user.achivesList.add(achiveRepo.findByName("endedChapter10"))
