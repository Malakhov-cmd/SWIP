...
class ElevenChapterProcessor(
    ...
    fun elevenChapterAnswers(
        ...
        val fourthChapter = javaLanguage.chapters[10]
    ...
    private fun checkerUserAnswerIfDefaultBehavior(
        ...
        val chapter = language.chapters[10]
    ...
    private fun checkToAddAchive(
        ...
        if (chapter.chapterProgress > 99) {
            if(user.achivesList.stream().filter{ it.name == "endedChapter13"  }.count() == 0L){
                user.achivesList.add(achiveRepo.findByName("endedChapter13"))
