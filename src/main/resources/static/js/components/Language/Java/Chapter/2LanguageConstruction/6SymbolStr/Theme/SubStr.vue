...
export default {
    ...
    Request() {
      ...
      const interval = setInterval(() => {
        if (isSendedandrecived) {
          ...
          window.frontendData.language.chapters[1].listThemes[18].task.tryCount++
          window.frontendData.language.chapters[1].listThemes[18].task.timeOnSolutionInSeconds += this.timeSpended
          ...
          this.$toasted.error("Некоректные данные! Время на решение: " + this.timeSpended +
              " c. Номер попытки: " + window.frontendData.language.chapters[1].listThemes[18].task.tryCount + ".",{
  ...
  beforeDestroy() {
    ...
    let themeToClose = document.getElementById('java-2-chapter-6-them-1-subtheme');
