    ...
    Request() {
      ...
      const interval = setInterval(() => {
        if (isSendedandrecived) {
          ...
          window.frontendData.language.chapters[1].listThemes[16].task.tryCount++
          window.frontendData.language.chapters[1].listThemes[16].task.timeOnSolutionInSeconds += this.timeSpended
          ...
          this.$toasted.error("Некоректные данные! Время на решение: " + this.timeSpended +
              " c. Номер попытки: " + window.frontendData.language.chapters[1].listThemes[16].task.tryCount + ".", {
        ...
        `class Main
{
  ...
  beforeDestroy() {
    ...
    let themeToClose = document.getElementById('java-2-chapter-5-them-8-subtheme');
